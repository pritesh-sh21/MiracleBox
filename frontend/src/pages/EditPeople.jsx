...
const EditPeople = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const peopleResponse = await fetch(`http://localhost:9000/api/people/${id}`).then(response => response.json());
                const sessionResponse = await fetch(`http://localhost:9000/api/community/session_details/people/${id}`).then(response => response.json());


                setValues({
                    name: peopleResponse.name,
                    age: peopleResponse.age,
                    gender: peopleResponse.gender,
                    fatherName: peopleResponse.fatherName,
                    motherName: peopleResponse.motherName,
                    phoneNumber: peopleResponse.phoneNumber,
                    address: peopleResponse.address,
                    dob: null,
                    selfEducation: peopleResponse.selfEducation,
                    childEducationStatus: peopleResponse.childEducationStatus,
                    employmentStatus: peopleResponse.employmentStatus,
                    welfareSchemes: peopleResponse.welfareSchemes,
                    familySize: peopleResponse.familySize,
                    dependentChildren: peopleResponse.dependentChildren,
                    elderlyFamilyMembers: peopleResponse.elderlyFamilyMembers,
    ...
    const updatePeople = async (people) => {
        console.log(people);
        return await fetch(`http://localhost:9000/api/people/${id}`, {
            method: "PUT",
            headers: {
    ...
    if (redirect) {
        return <Navigate to={`/community/session_details/${sessionId}`} />
    }
    return (
        <div className="p-2">
            <div>
